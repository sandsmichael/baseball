...
def _fetch_season_stats(name: str, season: int, is_pitcher: bool) -> tuple[dict | None, str | None]:
    """Fetch actual stats for a given season. Returns (stats_dict, fg_id) or (None, None)."""
    ...
    try:
        ...
        fg_id = str(int(r['IDfg'])) if 'IDfg' in r.index and r['IDfg'] is not None else None
        stats = result if any(v is not None for v in result.values()) else None
        return stats, fg_id
    except Exception:
        return None, None


def _get_pct_owned(oauth, league_key: str, player_key: str) -> float | None:
    """Return the Yahoo percent_owned value (0–100) for a player, or None."""
    try:
        data = _api_get(oauth, f'/league/{league_key}/players;player_keys={player_key};out=percent_owned')
        players = data['fantasy_content']['league'][1]['players']
        if not isinstance(players, dict) or not players.get('count', 0):
            return None
        p = players['0']['player']
        po_raw = {}
        if len(p) > 1 and isinstance(p[1], dict):
            po_raw = p[1].get('percent_owned', {})
            if isinstance(po_raw, list):
                po_raw = Yahoo._flat(po_raw)
        val = po_raw.get('value')
...
def lookup_player(q: str = Query(..., min_length=2)):
    ...
    try:
        ...
        # Availability per league + pct_owned from first league
        availability = []
        pct_owned: float | None = None
        for i, (_, lrow) in enumerate(leagues_df.iterrows()):
            lk = f"{game_key}.l.{lrow['league_id']}"
            ...
            if i == 0:
                pct_owned = _get_pct_owned(oauth, lk, player_key)

        # Current season stats + FanGraphs ID
        current_stats, fg_id = _fetch_season_stats(player_info['name'], SEASON, is_pitcher)
        current_stats = current_stats or {}

        # Historical stats — back to 2015 or until data runs out
        historical_stats = []
        consecutive_misses = 0
        for yr in range(SEASON - 1, 2014, -1):
            s, fid = _fetch_season_stats(player_info['name'], yr, is_pitcher)
            if s:
                historical_stats.append({'season': yr, 'stats': s})
                if not fg_id and fid:
                    fg_id = fid
                consecutive_misses = 0
            else:
