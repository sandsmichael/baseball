        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {data.pct_owned != null && (
                <span className="badge bg-indigo-100 text-indigo-800 font-semibold">
                  {data.pct_owned.toFixed(0)}% owned
                </span>
              )}
            </div>
            <div className="flex items-center gap-3 mt-0.5 flex-wrap">
              <p className="text-gray-500 text-sm">{p.team} · {p.positions}</p>
              {data.fg_id && (
                <a
                  href={`https://www.fangraphs.com/players/${p.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}/${data.fg_id}/stats`}
                  target="_blank"
