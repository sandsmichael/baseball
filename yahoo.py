...
def _load_creds(creds_file: str) -> dict:
    """
    Load credentials JSON, tolerating trailing garbage characters that
    yahoo_oauth sometimes appends (e.g. extra `}}`).
    Also rewrites the file if it was malformed so subsequent reads are clean.
    """
    with open(creds_file) as f:
        raw = f.read()
    try:
        ...
        # Strip trailing non-whitespace junk and retry
        obj, _ = json.JSONDecoder().raw_decode(raw.strip())
        # Rewrite the file clean so future reads work
        with open(creds_file, 'w') as f:
...
def top_available_all_leagues(
    ...
    def _fetch_league(league_key: str) -> pd.DataFrame:
        # Fetch currently claimable available players (status=A covers FA + processed waivers)
        data = _api_get(
            f'/league/{league_key}/players;status=A;count=100;out=percent_owned;sort=PO'
        )
        ...
        if isinstance(players, dict):
            for i in range(players.get('count', 0)):
                row = _parse_player_row(players[str(i)]['player'])
                rows.append(row)
                seen_keys.add(row['player_key'])

        # Supplement with players in the active waiver window (recently dropped, not yet claimable)
        # Yahoo excludes these from status=A until their waiver date passes.
        try:
            txn_data = _api_get(f'/league/{league_key}/transactions;count=25')
            txns = txn_data['fantasy_content']['league'][1]['transactions']
            pending_keys = []
            if isinstance(txns, dict):
                for i in range(txns.get('count', 0)):
                    t = txns[str(i)]['transaction']
                    t_flat = Yahoo._flat(t[0])
                    if t_flat.get('status') != 'successful':
                        continue
                    pd_data = t[1].get('players', {})
                    if not isinstance(pd_data, dict):
                        continue
                    for j in range(pd_data.get('count', 0)):
                        pl = pd_data[str(j)]['player']
                        pf = Yahoo._flat(pl[0])
                        td_raw = pl[1].get('transaction_data', [])
                        td = Yahoo._flat(td_raw) if isinstance(td_raw, list) else (td_raw or {})
                        if td.get('type') != 'drop':
                            continue
            ...
            # Batch-fetch ownership + percent_owned for pending players
            if pending_keys:
                keys_str = ','.join(pending_keys[:20])
                own_data = _api_get(
                    f'/league/{league_key}/players;player_keys={keys_str};out=percent_owned,ownership'
                )
                own_players = own_data['fantasy_content']['league'][1]['players']
                if isinstance(own_players, dict):
                    for i in range(own_players.get('count', 0)):
                        pl = own_players[str(i)]['player']
                        pf = Yahoo._flat(pl[0])
                        p1 = pl[1] if len(pl) > 1 else {}
                        own = p1.get('ownership', {})
                        if isinstance(own, list):
                            own = Yahoo._flat(own)
                        # Include if not currently owned by a team (handles waivers, FA, and transitional states)
                        if own.get('ownership_type') != 'team':
                            name_raw = pf.get('name', '')
                            name = name_raw.get('full', '') if isinstance(name_raw, dict) else str(name_raw)
                            rows.append({
                                'name':       name,
                                'team':       pf.get('editorial_team_abbr', ''),
                                'positions':  pf.get('display_position', ''),
                                'status':     pf.get('status', '') or '',
                                'pct_owned':  _parse_pct(p1),
                                'player_key': pf.get('player_key', ''),
                            })
        except Exception:
            pass

        if not rows:
            return pd.DataFrame(columns=['name', 'team', 'positions', 'status', 'pct_owned', 'player_key'])
        df = (
            pd.DataFrame(rows)
            .sort_values('pct_owned', ascending=False)
            .reset_index(drop=True)
        )
        return df[df['pct_owned'] >= 45].reset_index(drop=True)
...
def all_matchup_scores(
    leagues_df: pd.DataFrame,
    creds_file: str,
    season: int = None,
) -> list:
    """
    Return current week matchup scores across all leagues, with per-stat breakdown.

    Returns:
        List of dicts, one per league, with keys:
            league, league_id, my_team, opponent, week, start, end, status,
            scoring_method, my_points, opp_points,
            stats: [{stat_id, name, sort_order, mine, theirs, winning}]
    """
    season = season or date.today().year
    ...
    def _parse_val(v) -> float | None:
        """Parse Yahoo stat value; handles 'H/AB' format like '4/32'."""
        s = str(v).strip()
        if '/' in s:
            parts = s.split('/')
            try:
                denom = float(parts[1])
                return float(parts[0]) / denom if denom != 0 else 0.0
    ...
    for _, row in leagues_df.iterrows():
        ...
        try:
            # Get my team_key
            teams_data = _api_get(f'/league/{league_key}/teams')
            teams_raw  = teams_data['fantasy_content']['league'][1]['teams']
            my_team_key = None
            for i in range(teams_raw['count']):
                flat = Yahoo._flat(teams_raw[str(i)]['team'][0])
                if flat.get('is_owned_by_current_login') == 1:
                    my_team_key = flat['team_key']
                    break
            if not my_team_key:
                continue

            # Get stat category metadata: stat_id -> {name, sort_order}
            stat_meta: dict[str, dict] = {}
            try:
                settings   = _api_get(f'/league/{league_key}/settings')
                stat_cats  = settings['fantasy_content']['league'][1]['settings'][0] \
                ...
                for s in stat_cats:
                    ...
                    if sid:
                        stat_meta[sid] = {
                            'name':       st.get('display_name') or st.get('name', sid),
                            'sort_order': int(st.get('sort_order', 1)),
                        }
            except Exception as e:
                print(f'  stat_meta error ({league_name}): {e}')

            # Get scoreboard
            sb_data   = _api_get(f'/league/{league_key}/scoreboard')
            sb        = sb_data['fantasy_content']['league'][1]['scoreboard']
            week      = sb.get('week')
            matchups_raw = sb.get('0', {}).get('matchups', {})

            # Find my matchup by team_key
            my_match = None
            for i in range(matchups_raw.get('count', 0)):
                m     = matchups_raw[str(i)]['matchup']
                teams = m['0']['teams']
                for j in range(teams.get('count', 2)):
                    if Yahoo._flat(teams[str(j)]['team'][0]).get('team_key') == my_team_key:
                        my_match = m
                        break
                if my_match:
                    break
            ...
            my_stats_d:  dict[str, str] = {}
            opp_stats_d: dict[str, str] = {}
            my_pts:  str | None = None
            opp_pts: str | None = None
            opp_name = ''

            teams = my_match['0']['teams']
            for j in range(teams.get('count', 2)):
                team_data = teams[str(j)]['team']
                flat      = Yahoo._flat(team_data[0])
                is_mine   = flat.get('team_key') == my_team_key
                sub       = team_data[1] if len(team_data) > 1 else {}
                pts       = sub.get('team_points', {})
                pts_total = pts.get('total') if isinstance(pts, dict) else None
                stats_raw = sub.get('team_stats', {}).get('stats', [])
                stats_d   = {str(s['stat']['stat_id']): str(s['stat'].get('value', ''))
                             for s in stats_raw if isinstance(s.get('stat'), dict)}
                if is_mine:
                    my_stats_d = stats_d
                    my_pts     = str(pts_total) if pts_total is not None else None
                else:
                    opp_stats_d = stats_d
                    opp_pts     = str(pts_total) if pts_total is not None else None
                    opp_name    = Yahoo._name(flat.get('name', ''))

            # Build per-stat comparison list
            stat_list = []
            for sid, meta in stat_meta.items():
                mine   = my_stats_d.get(sid, '')
                theirs = opp_stats_d.get(sid, '')
                winning = None
                try:
                    mf = _parse_val(mine)
                    of = _parse_val(theirs)
                    if mf is not None and of is not None:
                        if meta['sort_order'] == 1:
                            winning = 'me' if mf > of else ('tie' if mf == of else 'them')
                        else:
                            winning = 'me' if mf < of else ('tie' if mf == of else 'them')
                except Exception:
                    pass
            ...
            results.append({
                ...
                'opponent':       opp_name,
                'week':           week,
                'start':          my_match.get('week_start', ''),
                'end':            my_match.get('week_end', ''),
            ...
            print(f'  matchup_scores error ({league_name}): {e}')
...
def il_overflow(
    ...
    """
    Scan all leagues and return players who ARE placed in an IL/DL fantasy slot
    but are no longer injured in real life (their MLB status is active/healthy).

    These players are blocking an IL slot that could be used for an actually
    injured player, or they should be moved to an active/bench slot.

    Returns:
        DataFrame with columns:
            league, my_team, name, mlb_team, positions, status, slot, player_key
    """
    _IL_STATUSES = {'IL', 'IL10', 'IL15', 'IL60', 'DL', 'DL15', 'DL60', 'DTD', 'Suspension', 'NA'}
    _IL_SLOTS    = {'IL', 'IL+', 'IL60', 'DL', 'NA', 'NA+'}
    ...
    for _, row in leagues_df.iterrows():
        ...
        try:
            ...
            for i in range(players['count']):
                ...
                status   = flat.get('status', '') or ''
                slot     = pos_flat.get('position', '')
                # In an IL slot but NOT injured
                if slot in _IL_SLOTS and status not in _IL_STATUSES:
                    rows.append({
                        'league':     league_name,
                        'my_team':    team_name,
                        'name':       Yahoo._name(flat.get('name', '')),
            ...
            print(f'    il_overflow error ({league_name}): {e}')
...
def benched_starters(
    leagues_df: pd.DataFrame,
    creds_file: str,
    season: int = None,
) -> pd.DataFrame:
    """
    Find pitchers (SP/RP/P) sitting in BN slots whose MLB team has a game today.
    Includes a `probable` flag for confirmed probable starters.

    Returns:
        DataFrame with columns:
            league, my_team, name, mlb_team, positions, status, slot, player_key, probable
    """
    _PITCHER_POS = {'SP', 'RP', 'P'}
    _BENCH_SLOTS = {'BN'}
    # Yahoo abbreviations that differ from MLB Stats API
    _ABBR_MAP = {'AZ': 'ARI', 'CHW': 'CWS'}

    today_str = date.today().isoformat()

    # Fetch today's MLB schedule + probable pitchers
    teams_with_games: set[str] = set()
    probable_names:   set[str] = set()
    try:
        sched = _requests.get(
            f'https://statsapi.mlb.com/api/v1/schedule?sportId=1&date={today_str}&hydrate=probablePitcher,team',
            timeout=10,
        ).json()
        for game_date in sched.get('dates', []):
            for game in game_date.get('games', []):
                for side in ('home', 'away'):
                    ...
                    if abbr:
                        teams_with_games.add(abbr)
                    pp = team_info.get('probablePitcher', {})
                    if pp.get('fullName'):
                        probable_names.add(pp['fullName'])
    except Exception as e:
        print(f'  benched-starters: MLB schedule fetch failed: {e}')
    ...
    for _, row in leagues_df.iterrows():
        ...
        try:
            ...
            data = _api_get(f'/team/{team_key}/roster;date={today_str}')
            players = data['fantasy_content']['team'][1]['roster']['0']['players']
            for i in range(players['count']):
                p = players[str(i)]['player']
                flat     = Yahoo._flat(p[0])
                ...
                if slot not in _BENCH_SLOTS:
                    continue
                if not set(positions.split(',')).intersection(_PITCHER_POS):
                    continue

                name = Yahoo._name(flat.get('name', ''))
                if name not in probable_names:
                    continue
            ...
            print(f'  benched-starters error ({league_name}): {e}')

    cols = ['league', 'my_team', 'name', 'mlb_team', 'positions', 'status', 'slot', 'player_key', 'probable']
    if not rows:
...
def upgrade_candidates(
    ...
    def _player_role(positions: str) -> str:
        """Return 'sp', 'rp', or 'batter' — SP and RP never compare against each other."""
        pos_set = set(str(positions).replace(' ', '').split(','))
        if 'SP' in pos_set:
            return 'sp'
        if 'RP' in pos_set or 'P' in pos_set:
            return 'rp'
        return 'batter'
    ...
    avail['_role'] = avail['positions'].map(_player_role)
    avail = (
        avail
        ...
        try:
            ...
            for _, p in yf.roster.iterrows():
                ...
                roster_rows.append({
                    ...
                    '_role':   _player_role(p['positions']),
                })
        except Exception as e:
...
class Yahoo:
    ...
    def _refresh(self):
        if _token_is_expired(self._creds_file):
